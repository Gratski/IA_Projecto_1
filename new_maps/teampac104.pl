...
pacman104(_,_,_,(Id,X,Y,_,M),_,Eles,_,_,_,_,_,Pasts,_,Dec) :-
	findall(D,(viz(D,(X,Y),Viz),member(Viz,Pasts),\+ naoSegura(Viz,Id,M,Eles)),L),
	random_member(Dec,L).

% se pastilhas numa casa a 2 passos vou para la (qualquer delas)
pacman104(_,_,_,(Id,X,Y,_,M),_,Eles,_,_,Free,_,_,Pasts,_,Dec) :-
	findall(D,(viz(D,(X,Y),Viz),member(Viz,Free),viz(_,Viz,VizViz),member(VizViz,Pasts),\+ naoSegura(Viz,Id,M,Eles)),L),
	random_member(Dec,L).

% se pastilhas numa casa a 3 passos vou para la (qualquer delas)
pacman104(_,_,_,(Id,X,Y,_,M),_,Eles,_,_,Free,_,_,Pasts,_,Dec) :-
	findall(D,(viz(D,(X,Y),Viz),member(Viz,Free),viz(_,Viz,VizViz),member(VizViz,Free), viz(_,VizViz,VizVizViz),member(VizVizViz,Pasts),\+ naoSegura(Viz,Id,M,Eles)),L),
	random_member(Dec,L).
...
pacman104(_,_,_,(_,X,Y,Dir,M),_,Eles,_,_,Free,_,_,_,_,Dec) :-
	findall(D,(viz(D,(X,Y),Viz),member(Viz,Free),\+ naoSegura(Viz,Id,M,Eles)),L),
	inversa(Dir,Inv),
	aoCalhaPrefiroNaoRegressar(L,Inv,Dec).
